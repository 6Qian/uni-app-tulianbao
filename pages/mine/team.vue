<template>
	<view class="content">
		...
		<view class="tops" style="z-index: 10;">
			<view class="userinfo">
				<view class="avatar">
					<image mode="widthFix" :src="info.avatar ? info.avatar : ''"></image>
				</view>
				<view class="text">
					<view class="ts">{{info.username}}&nbsp;&nbsp;&nbsp;{{info.team_title}}</view>
					<view class="tt">累计邀请了{{info.child || 0}}人</view>
				</view>
			...
			<view class="invdata" style="position: relative;" v-if="!modelsp">
				<image src="/static/kccbg.png" style="width: 720rpx; height: 166rpx; position: absolute; left: -15rpx; top: 0;" mode=""></image>
				<view class="lis" style="position: relative;">
					<view class="ts">{{info.today_child_reward || 0}}</view>
					<view class="tt" v-if="!modelsp">今日奖励</view>
					<view class="tt" v-else>今日收入金币</view>
				</view>
				<view class="lis" style="position: relative;">
					<view class="ts">{{info.child_reward || 0}}</view>
					<view class="tt" v-if="!modelsp">累计奖励</view>
		...
		<view class="myteam">
			<view class="title">
				<view class="left" v-if="!modelsp">我的团队</view>
				<view class="right" v-if="!modelsp">
					<view @click="setType(1)" :class="'lis '+(type == 1 ? 'active' : '') ">直推({{listinfo.child_1 || 0}})</view>
					<view @click="setType(2)" :class="'lis '+(type == 2 ? 'active' : '') ">有效({{listinfo.child_1_vip || 0}})</view>
					<view class="hr"></view>
					<view @click="setType(3)" :class="'lis '+(type == 3 ? 'active' : '') ">二级({{listinfo.child_2 || 0}})</view>
					<view @click="setType(4)" :class="'lis '+(type == 4 ? 'active' : '') ">有效({{listinfo.child_2_vip || 0}})</view>
				</view>
			</view>
...
<script>
	export default {
		...
		onLoad() {
			this.init();
			this.getList();
			console.log(this.modelsp,'this.modelsp')
		},
		methods: {
			init(){
				this.app.ajax({
			...
			getList() {
				this.app.ajax({
					url: "/share/child",
					data:{page:this.page,action:this.type},
					success:(res)=> {
						if(this.page == 1){
